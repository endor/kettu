Given /a torrent with the name "([^\"]+)"( and a download rate of "([^\"]+)" B\/s)?$/ do |name, has_download_rate, download_rate|
  ...
  File.open(file_path, 'w') do |f|
    f << '{"arguments": {"torrents":  [{"id":  4, "name": "' + name + '", "status": 4, "totalSize": 100, "sizeWhenDone": 100,"leftUntilDone": 50, "eta": 0, "uploadedEver": 0, "uploadRatio": 0, "rateDownload": ' + (has_download_rate ? download_rate : '0') + ',"rateUpload": 0, "metadataPercentComplete": 1, "addedDate": 27762987, "fileStats": []}]}}'
  end
end
...
Given /a torrent with an error "([^\"]+)"/ do |error|
  file_path = File.dirname(__FILE__) + '/../support/plural.json'
  File.open(file_path, 'w') do |f|
    f << '{"arguments": {"torrents":  [{"id":  4, "name": "my torrent", "status": 4, "totalSize": 100, "sizeWhenDone": 100,"leftUntilDone": 50, "eta": 0, "uploadedEver": 0, "uploadRatio": 0, "rateDownload": 0,"rateUpload": 0, "metadataPercentComplete": 1, "addedDate": 27762987, "fileStats": [], "error": 2, "errorString": "' + error + '"}]}}'
  end
end
